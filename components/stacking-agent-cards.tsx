...
export function StackingVirallookupCards() {
  const cardRefs = useRef<(HTMLDivElement | null)[]>([])
  // depth[i] = 0..N how many cards are currently stacked on top of card i
  const [depth, setDepth] = useState<number[]>(VIRALLOOKUPS.map(() => 0))

  useEffect(() => {
    function onScroll() {
      const nextDepth = VIRALLOOKUPS.map((_, i) => {
        // Count how many cards j > i are currently in sticky position (i.e. have scrolled past card i)
        let count = 0
        for (let j = i + 1; j < VIRALLOOKUPS.length; j++) {
  ...
  return (
    <div className="flex flex-col" style={{ perspective: "1400px", perspectiveOrigin: "50% 0%" }}>
      {VIRALLOOKUPS.map((VIRALLOOKUP, i) => {
        const d         = depth[i]
            ...
            >
              <div className="group relative bg-[#faf9f7] rounded-2xl border border-black/[0.07] overflow-hidden cursor-pointer">
                ...
                <div
                  ...
                  style={{ maxWidth: VIRALLOOKUP.img ? undefined : "100%" }}
                  // On desktop limit to left 60% so text doesn't overlap image
                >
                  <div className="md:max-w-[60%]">
                    <div className="flex items-start justify-between mb-6">
                      <Tag>{VIRALLOOKUP.label}</Tag>
                    </div>
                    <h3 className="text-xl font-light mb-3">{VIRALLOOKUP.title}</h3>
                    <p className="text-sm text-black/45 leading-relaxed mb-8">{VIRALLOOKUP.desc}</p>
                  </div>
                  <div className="flex gap-8 pt-6 border-t border-black/[0.06]">
                    {VIRALLOOKUP.stats.map(s => (
                      <div key={s.l}>
